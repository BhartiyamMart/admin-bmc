...
const DeliveryTimeSlotList = () => {
  ...
  const filteredSlots = useMemo(() => {
    return slots.filter((slot) => {
      ...
      const matchesStatus =
        statusFilter === 'all' ? true : statusFilter === 'active' ? slot.status === true : slot.status === false;
    ...
    { key: 'endTime', label: 'End Time' },
    {
      ...
      render: (slot: DeliverySlot) => (
        <button
          ...
          className={`rounded px-3 py-1 text-sm text-white ${slot.status ? 'bg-green-500' : 'bg-red-500'}`}
        >
    ...
    { key: 'sortOrder', label: 'Sort Order' },
    {
      ...
      render: (slot: DeliverySlot) => (
        <div className="flex justify-end gap-2 pr-4">
          ...
          <Trash2 className="w-5 cursor-pointer text-red-600" onClick={() => removeSlot(slot.id)} />
        </div>
      ),
    },
  ];
    ...
    <div className="bg-sidebar flex h-[calc(100vh-8vh)] justify-center p-4">
      <div className="w-full overflow-y-auto rounded-lg p-4 shadow-lg">
        ...
        <div className="w-full min-w-[300px] min-w-full sm:w-[560px] md:w-[640px] lg:w-[900px] xl:w-[1100px]">
          {/* Table */}
          <CommonTable<DeliverySlot> columns={columns} data={currentSlots} emptyMessage="No time slots found." />
