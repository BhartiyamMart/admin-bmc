...
export default function CreateOrder() {
  ...
  const addProduct = (product: Product, variant?: ProductVariant) => {
    ...
    const productName = variant 
      ? `${product.label.split(' - ')[0]} (${variant.name})`
                      ...
                      {offers.map((offer) => (
                        ...
                        >
                          <div className="flex w-full items-center justify-between">
                            ...
                            <Check className={cn('ml-auto', selectedOffer?.id === offer.id ? 'opacity-100' : 'opacity-0')} />
                          </div>
                          <span className="text-sm text-gray-500">
                            {offer.discountUnit === 'PERCENTAGE' ? `${offer.discountValue}% off` : `₹${offer.discountValue} off`}
                      ...
                      {coupons.map((coupon) => (
                        ...
                        >
                          <div className="flex w-full items-center justify-between">
                            ...
                            <Check className={cn('ml-auto', selectedCoupon?.id === coupon.id ? 'opacity-100' : 'opacity-0')} />
                          </div>
                          <span className="text-sm text-gray-500">
                            {coupon.discountUnit === 'PERCENTAGE' ? `${coupon.discountValue}% off` : `₹${coupon.discountValue} off`}
